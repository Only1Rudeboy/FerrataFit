...
/**
 * Die Vorarlberger Klettersteige.
 *
 ...
 * Ausgeschieden sind aus diesem Grund unter anderem der Alpenvereinssteig am
 * Zitterklapfen und die Bocksberg-Ueberschreitung.
 *
 * Zu den Zahlen: [climbMeters] meint die Hoehenmeter im gesicherten Steig, nie den
 * ganzen Tagesaufstieg. Die Unterscheidung ist wichtig, weil sonst ein langer Zustieg
 * eine kurze Wand zur Grosstour aufblaeht -- und die App daraus falsche Schluesse zieht.
 *
 * [verified] steht auf false, wo Quellen sich widersprachen. Die Anzeige weist dann
 * darauf hin. Das betrifft vor allem Schwierigkeitsgrade: Dieselbe Wand wird je nach
 * Seite als C oder als C/D gefuehrt, und beides ist ernst gemeint.
 *
 * Alle Angaben sind Anhaltspunkte fuer die Planung, kein Ersatz fuer den Blick vor Ort.
 */
object FerrataRoutes {

    val all: List<FerrataRoute> = listOf(
        FerrataRoute(
    ...
    fun byId(id: String): FerrataRoute? = all.firstOrNull { it.id == id }

    /** Alle Gebiete, in der Reihenfolge, in der sie im Katalog stehen. */
    val regions: List<String> get() = all.map { it.region }.filter { it.isNotBlank() }.distinct()
}
